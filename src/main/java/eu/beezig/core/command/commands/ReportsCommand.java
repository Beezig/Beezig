...
public class ReportsCommand implements Command {
    ...
    public boolean execute(String[] args) {
        boolean includeClaimed = args.length > 0 && args[0].toLowerCase(Locale.ROOT).startsWith("c");
        Beezig.net().getHandler().sendPacket(PacketReport.request(includeClaimed));
        return true;
    }

    public static void sendResult(ReportIncoming[] reports, ReportIncoming[] claimed) {
        ...
        Beezig.api().messagePlayer(StringUtils.linedCenterText(Color.primary(), Color.accent() + Beezig.api().translate("msg.page", "1")));
    }
}
