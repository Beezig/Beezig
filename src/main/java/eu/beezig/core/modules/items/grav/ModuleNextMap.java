/*
 * Copyright (C) 2017-2020 Beezig Team
 *
 * This file is part of Beezig.
 *
 * Beezig is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * Beezig is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Beezig.  If not, see <http://www.gnu.org/licenses/>.
 */

package eu.beezig.core.modules.items.grav;

import eu.beezig.core.Beezig;
import eu.beezig.core.server.HiveMode;
import eu.beezig.core.server.modes.GRAV;
import eu.beezig.core.util.ActiveGame;
import eu.the5zig.mod.modules.GameModeItem;
import eu.the5zig.mod.server.GameState;
import eu.the5zig.util.minecraft.ChatColor;

public class ModuleNextMap extends GameModeItem<GRAV> {

    public ModuleNextMap() {
        super(GRAV.class);
    }

    @Override
    public void registerSettings() {
        getProperties().addSetting("showcolors", true);
    }

    @Override
    protected Object getValue(boolean b) {
        if (b)
            return ChatColor.YELLOW + "Toys";
        HiveMode mode = ActiveGame.get();
        GRAV grav;
        if (!(mode instanceof GRAV) || (grav = (GRAV) ActiveGame.get()) == null)
            return "Error";
        return (boolean) getProperties().getSetting("showcolors").get() ?
            grav.getNextMap() : ChatColor.stripColor(grav.getNextMap());
    }

    @Override
    public boolean shouldRender(boolean dummy) {
        if (dummy) return true;
        return  super.shouldRender(false) && Beezig.api().isInWorld() &&
            getGameMode().getState() == GameState.GAME && getGameMode().getCurrentStage() < 5;
    }

    @Override
    public String getTranslation() {
        return "beezig.module.grav.next_map";
    }
}
