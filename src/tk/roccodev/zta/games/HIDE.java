...
public class HIDE extends GameMode {
	...
	public static void initDailyPointsWriter() throws IOException {
		File f = new File(ZTAMain.mcFile + "/hide/dailyPoints/" + dailyPointsName);
		if (!f.exists()) {
			f.createNewFile();
		...
		dailyPointsWriter = new PrintWriter(ZTAMain.mcFile + "/hide/dailyPoints/" + dailyPointsName, "UTF-8");
