		...
		// Votes]
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					parsedMaps.addAll(AutovoteUtils.getMapsForMode("gnt"));
					...
					public void run() {
						...
						try {
							...
							Double kd = Setting.Giant_SHOW_KD.getValue()
									? (double) Math.floor(((double) HiveAPI.GiantgetKills(Giant.lastRecords, lobby)
	...
	public void onTitle(Giant gameMode, String title, String subTitle) {
		if (subTitle != null) {
			...
			if (ChatColor.stripColor(subTitle).equalsIgnoreCase("Respawning in 3 seconds")) {
				Giant.gameDeaths++;
				Giant.gameKdr = ((double) ((double) Giant.totalKills + (double) Giant.gameKills)
						/ (double) ((double) Giant.gameDeaths + (double) Giant.totalDeaths == 0 ? 1
