...
class IOConnection implements IOCallback {
	...
	public void transportMessage(String text) {
		...
		case IOMessage.TYPE_EVENT:
			try {
				JSONObject event = (JSONObject) new JSONParser().parse(message.getData());
				Object[] argsArray;
				if (event.containsKey("args")) {
					JSONArray args = (JSONArray) event.get("args");
			...
			if (data.length == 2) {
				try {
					...
					if (ack == null)
						...
						JSONArray array = (JSONArray) new JSONParser().parse(data[1]);
